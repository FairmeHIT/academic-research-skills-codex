...
**Goal:** 将 ARS 从「git clone + symlink 到 `~/.claude/skills/`」的分发模式，升级成 Claude Code Plugin（`/plugin install` 一行装），同时保留 GitHub repo 作为 source of truth 与教学内容主入口。

**Architecture:** 双轨并行——同一个 repo 同时支援 (a) 传统 clone+symlink 安装、(b) Claude Code plugin marketplace 安装。Plugin 化只新增 `.claude-plugin/`、`commands/`、`agents/`、`hooks/` 四个目录，不动 `deep-research/`、`academic-paper/`、`academic-paper-reviewer/`、`academic-pipeline/` 四个既有 skill 目录的 SKILL.md 内容。

**Tech Stack:** Claude Code Plugin Manifest spec、JSON manifest、Markdown frontmatter（commands/agents）、JSON hooks config、既有 SKILL.md（不变）。

**Platform scope（必读）：**
- ✅ Claude Code CLI / VS Code 扩充 / JetBrains 扩充：plugin 完整支援
- ❌ claude.ai 网页版 / Claude for Work / Anthropic API 直呼：**完全不支援 plugin**，这条路继续靠 GitHub README + 教学内容
- ❌ Codex CLI：用 `academic-research-skills-codex-zh-CH` fork，不在本 roadmap 范围
...
**验收：** 在干净的 Claude Code session 跑 `/plugin install`，4 个 skill 出现在 skill 清单，`/skill deep-research` 可正常触发。
...
Expected: Claude Code 启动后 `/skill list` 看得到 deep-research / academic-paper / academic-paper-reviewer / academic-pipeline
...
```bash
git add .claude-plugin/ skills/ README.md README.zh-CN.md
...
**验收：** 在另一台机器 `/plugin marketplace add Imbad0202/academic-research-skills` 后开 auto-update，repo push 新 commit 后重启 Claude Code 自动拉新版。
...
| Codex CLI 用户误装 plugin | 低 | README 明写「plugin = Claude Code only；Codex 用 academic-research-skills-codex-zh-CH」 |
