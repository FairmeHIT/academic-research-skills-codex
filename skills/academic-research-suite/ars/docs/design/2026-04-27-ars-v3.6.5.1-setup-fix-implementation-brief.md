...
- `docs/SETUP.zh-CN.md` if it exists (mirror EN changes)
- `README.md` and `README.zh-CN.md` install method sections (sweep, only if they mention Cowork or claude.ai install)
...
grep -rn -E "Cowork|claude\.ai|claude\.com/product/cowork|GitHub integration|\.claude/skills" \
  README.md README.zh-CN.md QUICKSTART.md .claude/CLAUDE.md docs/ \
  --exclude-dir=design
```
...
- `README.zh-CN.md` — same as English
...
- `docs/SETUP.zh-CN.md` — mirror EN changes if zh-CN exists (may or may not — check first)
