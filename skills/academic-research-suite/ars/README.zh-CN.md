...
> Claude-only 功能，在 Codex 版只会被模拟或明确列为不支援。
...
- *选用：* Pandoc 用于 DOCX 输出，tectonic + 思源宋体 SC 用于 APA 7.0 PDF（纯 Markdown 输出两个都不需要）
...
**👉 [docs/PERFORMANCE.zh-CN.md](docs/PERFORMANCE.zh-CN.md)** — 各模式 token 预算、完整 pipeline 估算（~$4–6 for 一篇 15k 字论文），以及建议的 Claude Code 设定（Skip Permissions；Agent Team 选用）。

## 使用指南与文章

- [学术写作不该是一个人的事：一套开源 AI 协作工具如何改变研究者的工作流](https://open.substack.com/pub/edwardwu223235/p/ai?r=4dczl&utm_medium=ios) — 完整使用指南（简体中文）
...
>
> 不过，一般的 `Trigger Keywords` 区块（决定 skill 是否被启动）仍以英文和简体中文为主。如果你发现 skill 在你的语言下触发不稳定，可以在各 `SKILL.md` 的 `### Trigger Keywords` 区块中加入你的语言的关键字，提高匹配信心。
...
- **跨模型验证**：设定 `ARS_CROSS_MODEL` 启用第二 AI 模型独立审查。详见 [docs/SETUP.zh-CN.md](docs/SETUP.zh-CN.md)。
...
**[cloudenochcsis](https://github.com/cloudenochcsis)** — 贡献者。将 IS 章节从 *Basket of 8* 扩充为完整的 *Senior Scholars' Basket of 11*，补上 *Decision Support Systems*、*Information & Management*、*Information and Organization*（[Issue #7](https://github.com/Imbad0202/academic-research-skills/issues/7)、[PR #8](https://github.com/Imbad0202/academic-research-skills/pull/8)）。资料来源：[AIS Senior Scholars' List of Premier Journals](https://aisnet.org/page/SeniorScholarListofPremierJournals)。
...
- **#104 — README motivation：Zhao 等人 corpus-scale 证据锚点**（2026-05-15）：README + `README.zh-CN.md` 动机段以 Zhao 等人 146,932 笔幻觉引用的发现作为 v3.7.x 线设计动机的证据锚点。
...
> Plugin 打包升级：ARS 现可在 Claude Code CLI / VS Code / JetBrains 一行装（`/plugin marketplace add Imbad0202/academic-research-skills` + `/plugin install academic-research-skills`）。原本的 `git clone + symlink 到 ~/.claude/skills/` 安装流程不变、继续支援；双轨都是一级公民。

- **Plugin manifest 与 marketplace metadata**（Phase 1，PR #68）：`.claude-plugin/plugin.json` 宣告整个 suite（4 个 skill 透过 `skills/` 目录相对 symlink 自动探索）；`.claude-plugin/marketplace.json` 注册 plugin，使单一 GitHub-hosted endpoint 同时提供 marketplace listing 与 plugin 来源。README、`README.zh-CN.md`、`docs/SETUP.md` 都加入双轨安装指引。
...
- **`docs/PERFORMANCE.md` + `.zh-CN.md`** 新增「v3.7.0 Plugin agent 与模型路由」节，说明 inherit 语意与目前 3-agent scope 边界。
...
- **Opt-in passport 重置边界**（`ARS_PASSPORT_RESET=1`）。把每个 FULL checkpoint 提升为 context 重置边界。新增 `resume_from_passport=<hash>` 模式，让使用者在新的 Claude Code session 单凭 Material Passport ledger 就恢复 pipeline，不重播先前对话。`systematic-review` 模式 flag ON 时，每个 FULL checkpoint 一律强制重置；其他模式视重置为 flag 开启后的强预设。Flag OFF 时 byte-for-byte 维持 pre-v3.6.3 行为。
...
- `docs/PERFORMANCE.zh-CN.md` 更新 long-running session 指引。
- 无破坏性变更，flag 预设关闭。
...
- 新增 `docs/SETUP.md` / `docs/SETUP.zh-CN.md`（前置需求、API key、Pandoc/tectonic、跨模型验证、四种安装方式），以及 `docs/PERFORMANCE.md` / `docs/PERFORMANCE.zh-CN.md`（token 预算、建议 Claude Code 设定）。README 以连结取代内嵌。
...
- 同步 `README.md` 与 `README.zh-CN.md` 内嵌的 changelog 区块，补上原本缺漏的 `v3.3.3` 与 `v3.3.2` 发版摘要。
